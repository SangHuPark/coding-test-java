 ...
 *  -- 세로일 때, col + 1 이 맵을 벗어나지 않고 (row-1/row/row+1, col+1)  빈칸인 경우만 true
 * - canTURN()
 *  -- 가로일 때, row - 1 가 맵을 벗어나지 않고 (row-1, col-1/col/col+1) 이 빈칸이고
 *              row + 1 이 맵을 벗어나지 않고 (row+1, col-1/col/col+1) 이 빈칸인 경우만 true
 *  -- 세로일 때, col - 1 이 맵을 벗어나지 않고 (row-1/row/row+1, col-1) 이 빈칸이고
 *              col + 1 이 맵을 벗어나지 않고 (row-1/row/row+1, col+1) 이 빈칸인 경우만 true
 * - isDefinition()
